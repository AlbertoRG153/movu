...
const getLocalStorageWithExpiry = (key: string) => {
  if (typeof window !== 'undefined') {
    ...
    try {
      ...
      console.log(`Error parsing JSON for key ${key}, returning raw value`);
      return itemStr;
    }
  }
  return null;
...
export default function VehiculoPage() {
  ...
  const isFormComplete =
    selectedVehicleType && plateNumber && brand && model && color && imagePreview;
  ...
  const handleSubmit = () => {
    ...
    if (imagePreview) {
      ...
      console.log("✅ Imagen guardada en localStorage (expira en 24 horas)");
    }

    // Mostrar en consola
    console.log("✅ Datos guardados en localStorage (expiran en 24 horas):");
  ...
  return (
    <div className="min-h-screen bg-[#0D3A45] text-white flex flex-col items-center px-4">
      <div className="h-[10vh] w-full mt-5 text-center">
        <h1 className="text-xl text-white font-semibold">Información acerca del vehículo</h1>
      </div>
      ...
      {/* Marca */}
      <div className="bg-white text-black p-6 rounded-lg w-11/12 max-w-md mb-4">
        <p className="text-sm mt-2 text-justify mb-3">Ingresa la marca de tu vehículo.</p>
        <input
          type="text"
          value={brand}
      ...
      {/* Modelo */}
      <div className="bg-white text-black p-6 rounded-lg w-11/12 max-w-md mb-4">
        <p className="text-sm mt-2 text-justify mb-3">Ingresa el modelo de tu vehículo.</p>
        <input
          type="text"
          value={model}
      ...
      {/* Color */}
      <div className="bg-white text-black p-6 rounded-lg w-11/12 max-w-md mb-4">
        <p className="text-sm mt-2 text-justify mb-3">Ingresa el color del vehículo.</p>
        <input
          type="text"
          value={color}
